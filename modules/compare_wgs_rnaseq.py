...
Now supports analysis mode:
  - 'all' : use all common sites between RNA and WGS.
  - 'verifybamid' : restrict to VerifyBAMID SNP panel (faster, focuses on key sites).
...
- **Three‑tier fallback**:
    ...
    3. Position‑list method (extract positions, intersect, then **merge** both subsets) – guaranteed to work.
...
def run(rna_vcf, wgs_vcf, sample_map, gene_expression, exome_bed, output_dir, stats_path, params):
    ...
    Args:
        ...
        params (dict): Module parameters from config:
            ...
            - analysis_mode (str): 'all' (use all common sites) or 'verifybamid' (restrict to VerifyBAMID SNPs).
    ...
    # Check for verifybamid mode and dynamically set the restrict BED file
    analysis_mode = params.get('analysis_mode', 'all')
    restrict_bed = None
    if analysis_mode == 'verifybamid':
        verifybamid_snps_file = os.path.join(output_dir, 'variant_qc', 'verifybamid_snps.tsv')
        if not os.path.exists(verifybamid_snps_file):
            logger.error("verifybamid_snps.tsv not found at %s. Please run 'variant_qc' module first.", verifybamid_snps_file)
            raise FileNotFoundError(f"VerifyBAMID SNPs file missing for analysis_mode='verifybamid'")
        restrict_bed = verifybamid_snps_file
        logger.info(f"  Using VerifyBAMID SNP panel for site restriction: {restrict_bed}")
    ...
    # -------------------------------------------------------------------------
    # 6. Generate the common sites VCF (intersection of RNA and WGS)
    #    If analysis_mode == 'verifybamid', we restrict to the VerifyBAMID SNP list.
    # -------------------------------------------------------------------------
    ...
    if utils.is_file_up_to_date(common_vcf, dependencies):
        ...
        logger.info("  Generating common VCF (sites present in both RNA and WGS)...")
        try:
            _intersect_vcfs_parallel(rna_vcf, wgs_vcf, common_vcf, params, restrict_bed)
    ...
    # -------------------------------------------------------------------------
    # 12. Coverage correlation (fixed to use per‑sample DP)
    # -------------------------------------------------------------------------
...
def _extract_genotypes(vcf, sample_pairs, out_tsv, params):
    ...
    # Query GT, DP, GQ for all samples using correct format for FORMAT fields
    ...
    for line in result.split('\n'):
        ...
        for i, (rna_s, wgs_s) in enumerate(sample_pairs):
            # Data for RNA sample: at position 3*rna_idx[i], 3*rna_idx[i]+1, 3*rna_idx[i]+2
...
def _ase_analysis(vcf, sample_pairs, out_dir, params):
    ...
    Uses the AD (allele depth) field from FORMAT.
    """
    logger = utils.get_logger()
...
def _coverage_correlation(vcf, sample_pairs, out_dir):
    ...
    for rna, wgs in sample_pairs:
        ...
        for line in result.split('\n'):
            ...
            parts = line.split()
            if len(parts) >= 4:
                chrom, pos, dp_rna_str, dp_wgs_str = parts[0], parts[1], parts[2], parts[3]
                # Handle missing or invalid DP values
                try:
                    dp_rna = int(dp_rna_str) if dp_rna_str != '.' else -1
                except ValueError:
                    dp_rna = -1
                try:
                    dp_wgs = int(dp_wgs_str) if dp_wgs_str != '.' else -1
                except ValueError:
                    dp_wgs = -1
                if dp_rna > 0 and dp_wgs > 0:
                    dp_pairs.append((dp_rna, dp_wgs))
        if len(dp_pairs) > 1:
...
# -----------------------------------------------------------------------------
def _intersect_vcfs_parallel(rna_vcf, wgs_vcf, out_vcf, params, restrict_bed=None):
    """
    Parallel chromosome‑wise intersection of two VCFs with multiple fallback strategies.
    If restrict_bed is provided (path to a BED file), the intersection is further restricted to those positions.
    ...
    def try_position_list():
        ...
        # If restrict_bed is provided, further filter intersect_pos to those positions
        if restrict_bed is not None:
            logger.info(f"    Restricting to positions from {restrict_bed}...")
            restrict_df = pd.read_csv(restrict_bed, sep='\t', header=None, names=['chrom','pos','ref','alt'])
            # The file has columns: chrom, pos, ref, alt (1-based positions)
            restrict_pos = set()
            for _, row in restrict_df.iterrows():
                chrom = row['chrom']
                pos = row['pos']
                restrict_pos.add((chrom, pos))
            orig_size = len(intersect_pos)
            intersect_pos = intersect_pos & restrict_pos
            logger.info(f"      After restriction: {len(intersect_pos)} positions (removed {orig_size - len(intersect_pos)}).")
